#!/usr/bin/env python3
"""
Convert soarm-pi05-state-11997 JAX checkpoint to PyTorch safetensors.

Uses Pi0Config(pi05=True) to match Pi05Config architecture,
bypassing the isinstance check in convert_jax_model_to_pytorch.py.
"""
...
CHECKPOINT_DIR = str(Path.home() / ".cache/openpi/checkpoints/soarm-pi05-state-11997")
OUTPUT_PATH = str(Path.home() / ".cache/lerobot/converted/soarm-pi05-state-11997-pytorch")

# Pi05Config(soarm_pi05_flow_lora) uses:
#   action_horizon=50, action_dim=32, paligemma_variant="gemma_2b_lora"
# Checkpoint has full-rank weights (LoRA merged), so we use gemma_2b for PyTorch model.
...
# Fix tied weights: safetensors.save_model only saves lm_head.weight (one copy),
# but embed_tokens.weight is needed separately for load_state_dict to work without tie_weights().
