...
if _JAX_AVAILABLE:
    ...
    def resize_with_pad(images, height, width, method=None):  # type: ignore[misc]
        raise RuntimeError("resize_with_pad (JAX) is not available without JAX installed.")
...
def resize_with_pad_torch(
    ...
    if channels_last:
        ...
        if added_batch_dim:
            padded_images = padded_images.squeeze(0)  # Remove batch dimension only if we added it
